...
SEVERITY_MEDIUM = 2
SEVERITY_HIGH = 3
SEVERITY_CHOICES = collections.OrderedDict([("info", SEVERITY_INFO),
                                            ("low", SEVERITY_LOW),
...
SEVERITY_PRINT = {SEVERITY_DROP: "DROP ",
                  SEVERITY_INFO: "INFO ",
                  SEVERITY_LOW: "LOW  ",
                  SEVERITY_UNKNOWN: "UNKN ",
                  SEVERITY_MEDIUM: "MED  ",
                  SEVERITY_HIGH: "HIGH "}
        ...
        with open(self.conf_file) as ffile:
            for nline, line in enumerate(ffile):
                ...
                try:
                    ...
                    if len(e) < 3:
                        raise Exception()
                    if e[CONF_SOURCE] not in self.conf:
                        self.conf[e[CONF_SOURCE]] = dict()
                    if e[CONF_ACTION] in SEVERITY_CHOICES or e[CONF_ACTION] == "drop":
                        ...
                        # XXX bug res does not exist
                        if len(res) < 2:
                            raise Exception()
                        if sev not in self.conf[e[CONF_SOURCE]]:
                            self.conf[e[CONF_SOURCE]][sev] = dict()
